  ...
  <main id="app">
    ...
    <TopHead v-if="app && messages.length > 0" :app="app">
      ...
      <button
        ...
        :title="
          muted
            ? (config.i18n[lang()] && config.i18n[lang()].unMuteTitle) ||
              config.i18n[config.app.fallback_lang].unMuteTitle
            : (config.i18n[lang()] && config.i18n[lang()].muteTitle) ||
              config.i18n[config.app.fallback_lang].muteTitle
        "
        :aria-label="
          muted
            ? (config.i18n[lang()] && config.i18n[lang()].unMuteTitle) ||
      ...
      <section v-else aria-live="polite">
        <div v-for="m in messages" :key="m.responseId" class="message">
          ...
          /></BubbleWrapper>
            ...
            <Bubble  :text="m.queryResult.fulfillmentText" />

          <!-- Dialogflow Components -->
          <div
            v-for="(component, id) in m.queryResult.fulfillmentMessages"
            :key="id"
            class="component"
          >
          ...
          <section
            v-if="
              m.queryResult.webhookPayload &&
                m.queryResult.webhookPayload.google
            "
          >
            <div
              v-for="(component, id) in m.queryResult.webhookPayload.google
                .richResponse.items"
              :key="id"
              class="component"
            >
...
const  clientUrl =  process.env.VUE_APP_SERVER_URL
export default {
  name: "App",
  components: {
    Welcome,
  ...
  created() {
    ...
    if (this.history() && localStorage.getItem("agent") !== null) {
      ...
      let agent = {
        ...
        description:
          'Choose your language and send me a "Hi" to get started!',
        avatarUri: "goku991.png",
        enableLogging: true,
  ...
  methods: {
    send(q) {
      ...
      axios
        .post(`${clientUrl}askQuery`, request)
        .then(res => {
